...
class DSN(nn.Module):
    def __init__(self,BERT):
        ...
        self.Domain_classifier.add_module("fc1", nn.Linear(self.config.hidden_size, self.config.hidden_size))
        self.Domain_classifier.add_module("relu", nn.ReLU(True))
        self.Domain_classifier.add_module("fc2", nn.Linear(self.config.hidden_size, 2))
    ...
    def forward(self,input_ids,attention_mask,mode,p=0.0):
        if mode =="eva":
            ...
            #get domian_logits
            reversed_share_cls = ReverseLayerF.apply(share_cls, p)
            domain_logits = self.Domain_classifier(reversed_share_cls)
            # domain_logits = [batch，max——seq，2]
