...
from functions import mmd_rbf_noaccelerate,DiffLoss
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, (src, tgt) in data_zip:
            ...
            for i in range(args.max_seq):
                tgt_dif_loss =tgt_dif_loss+ Difloss(tgt_output["private_feature"][:,i,:],
                                                    tgt_output["share_feature"][:,i,:])
            tgt_dif_loss = (args.dif_weight/args.max_seq) * tgt_dif_loss
            #src
            src_dif_loss=0
            for i in range(args.max_seq):
                src_dif_loss =src_dif_loss+ Difloss(src_output["private_feature"][:,i,:],
                                                    src_output["share_feature"][:,i,:])
            ...
            if epoch >= args.mmd_epoch:
                sim_loss = args.sim_weight*(mmd_rbf_noaccelerate(src_output["pooled_share_feature"],tgt_output["pooled_share_feature"]))
        ...
        if valid_F>bestf1:
            save_model(net=dsn,name=args.src+"-"+args.tgt+"-FRSE-mmd")
            bestf1 =valid_F
